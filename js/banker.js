...
function banker() {
    ...
    while ( allocationMatrix.length != 0 ) {
        ...
        for ( i = 0; i < allocationMatrix.length; i++ ) {
            test = true;
            for ( j = 0; j < allocationMatrix[0].length; j++ ) {
                if ( terminated[i] == true && demandMatrix[i][j] - allocationMatrix[i][j] > A[j] ) {
                    test = false;
                    j = allocationMatrix[0].length;
                }
            }
            if ( test == true ) {
                current_process = i;
        ...
        if ( current_process != null ) {
            ...
            allocationMatrix[current_process] = demandMatrix[current_process];
            ...
            allocationMatrix.check[ current_process ] = false;
            stateStack.push({
                P: P,
                A: A,
                allocationMatrix: allocationMatrix,
